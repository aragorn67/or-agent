...
**Routing fix applied (the cheap action the data implied).** Broadened
the two LLM-free routers: `detect_analysis_type_keyword_based` gained
`sensitiv` (covers "sensitive"), "how does"/"responsive to", and the
"what changes/happens if/when" phrasings; `_check_deterministic_intent`
follow-up patterns gained suppose/imagine/rerun/redo (context-gated, so
no first-message blast radius). **Deliberately NOT closed:** bare
modification imperatives ("drop P2 by 20%", "bump Boston to 1500") stay
LLM-routed — keyword-forcing them would misroute partial new-problem
*first* messages into the no-baseline guide (`_analysis_needs_baseline`,
core.py:684); deferring is correct, not a gap. Re-measured on the
matrix: analysis-type escalations 4→2 (the 2 are the principled
deferrals), intent unchanged at 3 (first-message no-context, correct),
+2 context follow-up phrasings now deterministic. Full suite **248
passed**, same documented baseline — zero regressions (incl.
`test_followup_whatif` / `test_first_message_analysis`, the
`detect_analysis_type` blast-radius tests). Net: the free layer's
residual escalation is small *and principled*, not a "need a bigger
model" signal — question answered with action, no new component.
